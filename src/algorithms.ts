...
export const MOVE_TOKEN_REGEX = /([RULDFBruldfb][wW]?(?:2|²|')?|[MESmesxyzXYZ](?:2|²|')?|[()])/g;
...
export function invertSequenceStr(seq: string): string {
  const tokens = seq.trim().match(MOVE_TOKEN_REGEX) || [];
...
export function parseSequence(seq: string): AlgoStep[] {
  const tokens: string[] = seq.trim().match(MOVE_TOKEN_REGEX) || [];
